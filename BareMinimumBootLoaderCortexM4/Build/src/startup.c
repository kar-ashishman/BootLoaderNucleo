...
/*_la_data is pointing to start of data in flash in linker*/
...
void Reset_Handler(void) {
	...
	//set the Vector table address to 0x08000000
    uint32_t size, *dptr, *sptr;

    // disable interrupts
    __asm volatile ("CPSID i");

    // set VTOR register to look at the bootloaders vector table
    // vector table base address is fetched from linker using _vector_table
    volatile uint32_t *vtor = (uint32_t*)VTOR_ADDR;
    *vtor = (uint32_t)&_vector_table;

    // Pipe line scheduling.
    // Following instructions ensure all prefetched instructions are evaluated before moving to next instructions
    __asm volatile ("DSB");
    __asm volatile ("ISB");

    /* copy .data to SRAM */
    dptr = (uint32_t *)&_sdata;
    sptr = (uint32_t *)&_la_data;
...
// Default handler for all interrupts and exceptions to be overwritten later
// the weak attribute makes the function eligible to be re-defined.
static void Default_Handler(void) { while(1); }


// List of interrupts, exception handlers - to be redefined if needed
...
// Placing the pointers inside vector table. This will be placed in the correct location by linker
// the array is forced to be inside a section called .vector_table and linker places the section on top of the flash
uint32_t __attribute__((section(".vector_table"))) vector[] = {
    ...
    (uint32_t)&RTC_Alarm_IRQHandler,            // 38
    ...
    (uint32_t)&TIM1_UP_TIM10_IRQHandler,                   // 51
    (uint32_t)&USART3_IRQHandler,                   // 52
    (uint32_t)&EXTI15_10_IRQHandler,                // 53
    (uint32_t)&USART2_IRQHandler,                // 54
